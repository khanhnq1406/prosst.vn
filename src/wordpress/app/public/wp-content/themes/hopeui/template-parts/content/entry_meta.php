	...
	<ul class="list-inline">
		...
		if (!empty($parent_string)) { ?>
			<li class="posted-in">
				...
				$parent_note = _x('In %s', 'post parent', 'hopeui');
		...
		if ($postcat) {
			foreach ($postcat as $cat) { ?>
				<li class="widget_categories"><a href="<?php echo esc_url(get_category_link($cat->cat_ID)) ?>"><?php echo esc_html($cat->name); ?></a></li>
			<?php }
		}
		...
		if (!empty($author_string)) { ?>
			<li class="posted-by">
				<?php
				echo esc_html(get_theme_mod('hopeui_php_blog_author_by_text'));
				/* translators: %s: post author */
				$author_byline = _x('By %s', 'post author', 'hopeui');
				if (!empty($time_string)) {
					/* translators: %s: post author */
					$author_byline = _x('%s', 'post author', 'hopeui');
				}
				printf(
