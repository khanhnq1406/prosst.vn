		...
		<ul class="woocommerce-mini-cart cart_list product_list_widget p-0 <?php echo esc_attr($args['list_class']); ?>">
			...
			foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
				...
				if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key)) {
					...
					$remove_item_link = apply_filters(
						...
						sprintf(
							'<a href="%s" class="remove remove_from_cart_button" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s"><i class="fas fa-trash"></i></a>',
							esc_url(wc_get_cart_remove_url($cart_item_key)),
							esc_attr__('Remove this item', 'hopeui'),
							esc_attr($product_id),
			...
			?>
					<li class="woocommerce-mini-cart-item <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
						...
						<div class="hopeui_style-cart-img">
						...
						<div class="hopeui_style-cart-content">
							<?php
							echo wp_kses_post($remove_item_link);
							?>
							<a class="d-block" href="<?php echo esc_url($product_permalink); ?>">
								<h6 class="hopeui_style-product-title"><?php echo esc_html($product_name); ?></h6>
							</a>
...
<?php else : ?>
	...
	<p class="woocommerce-mini-cart__empty-message no-request m-0"><?php esc_html_e('No products in the cart.', 'hopeui'); ?></p>
