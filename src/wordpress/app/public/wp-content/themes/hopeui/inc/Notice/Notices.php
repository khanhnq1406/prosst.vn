...
class Notice
{
	...
	public function the_notice()
	{
		...
		wp_register_style('hopeui-notice', false);

		wp_add_inline_style('hopeui-notice', '
		.hopeui_style-notice-wrapper {
            display: flex;
            align-items: center;
			gap: 1em;
        }
        .hopeui_style-notice-wrapper .hopeui_style-logo img {
            min-width: 35px;
        }
		
		');
		wp_enqueue_style('hopeui-notice');


		$html = sprintf('<div class="hopeui_style-notice-wrapper"><div class="hopeui_style-logo"><img src="%s" alt=""></div><div class="hopeui_style-body">%s%s</div></div>',  get_stylesheet_directory_uri() . '/assets/images/redux/logo.svg', $this->get_title(), $this->get_message());
